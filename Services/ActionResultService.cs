...
public class ActionResultService
{
    ...
    public IActionResult GetAction(Results result, string? message = null, object? content = null)
    {
        return result switch
        {
            ...
            Results.Bad          => new BadRequestObjectResult(ResponseObject(message, 400, content)),
            Results.Unauthorized => new UnauthorizedObjectResult(ResponseObject(message, 401, content)),
            Results.ServerError  => new ObjectResult(ResponseObject(message, 200, content)) { StatusCode = 500 },
            _ => new ObjectResult(ResponseObject(message, 200, content)) { StatusCode = 500 },
        };
    }
    ...
    public IActionResult GetActionAuto(Results result, string featur = "", object? content = null)
    {
        ...
        return result switch
        {
            ...
            Results.Delete       => new OkObjectResult(ResponseObject($"O Recurso{featur} foi acessado, mas está vazio...", 200)),
            Results.NoContent    => new OkObjectResult(ResponseObject($"O Recurso{featur} foi atualizado com sucesso!", 200, content)),
            ...
            Results.Unauthorized => new UnauthorizedObjectResult(ResponseObject
                ($"Você não tem autorização para acessar o recurso{featur}!", 401, content)),
            Results.ServerError  => new ObjectResult(ResponseObject("Erro interno do servidor!", 500, content)) { StatusCode = 500 },
            _ => new ObjectResult(ResponseObject("Um erro inesperado ocorreu no servidor...", 500, content)) { StatusCode = 500 }
        };
    }
